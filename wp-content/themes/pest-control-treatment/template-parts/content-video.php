...
<?php
  $pest_control_treatment_content = apply_filters( 'the_content', get_the_content() );
  $pest_control_treatment_video = false;

  // Only get video from the content if a playlist isn't present.
  if ( false === strpos( $pest_control_treatment_content, 'wp-playlist-script' ) ) {
    $pest_control_treatment_video = get_media_embedded_in_content( $pest_control_treatment_content, array( 'video', 'object', 'embed', 'iframe' ) );
  }
?>
  ...
  <div class="post-main-box p-3 mb-3 wow zoomIn" data-wow-duration="2s">
    <?php
      if ( ! is_single() ) {
        ...
        if ( ! empty( $pest_control_treatment_video ) ) {
          foreach ( $pest_control_treatment_video as $pest_control_treatment_video_html ) {
            echo '<div class="entry-video">';
              echo $pest_control_treatment_video_html;
            echo '</div>';
          }
        };
      };
