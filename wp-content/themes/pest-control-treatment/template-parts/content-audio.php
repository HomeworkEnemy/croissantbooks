...
<?php
  ...
  $pest_control_treatment_audio = false;

  // Only get audio from the content if a playlist isn't present.
  if ( false === strpos( $content, 'wp-playlist-script' ) ) {
    $pest_control_treatment_audio = get_media_embedded_in_content( $content, array( 'audio' ) );
  }
?>
  ...
  <div class="post-main-box p-3 mb-3 wow zoomIn" data-wow-duration="2s">
      <?php
        if ( ! is_single() ) {
          ...
          if ( ! empty( $pest_control_treatment_audio ) ) {
            foreach ( $pest_control_treatment_audio as $pest_control_treatment_audio_html ) {
              echo '<div class="entry-audio">';
                echo $pest_control_treatment_audio_html;
